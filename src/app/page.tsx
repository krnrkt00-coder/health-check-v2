...
const STORAGE_FALLBACK_PEOPLE = ["佐藤", "鈴木", "高橋", "田中"];
const DEFAULT_PEOPLE = STORAGE_FALLBACK_PEOPLE;
...
const emptyForm = (person = DEFAULT_PEOPLE[0], date = today()) => ({
  person,
  date,
...
function ChipButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-full px-3 py-1.5 text-sm ring-1 transition ${active ? "bg-slate-900 text-white ring-slate-900" : "bg-slate-100 text-slate-700 ring-slate-200 hover:bg-slate-200"}`}
    >
      {children}
    </button>
  );
...
async function fetchSharedState(): Promise<AppState> {
  try {
    const res = await fetch("/api/state", { cache: "no-store" });
    if (!res.ok) return emptyState;
    const data = (await res.json()) as Partial<AppState>;
    return {
      people: Array.isArray(data.people) && data.people.length > 0 ? data.people : DEFAULT_PEOPLE,
...
async function persistSharedState(state: AppState) {
...
export default function Home() {
  ...
  const [form, setForm] = useState(() => emptyForm(DEFAULT_PEOPLE[0]));

  useEffect(() => {
    let cancelled = false;
    (async () => {
      const next = await fetchSharedState();
      if (cancelled) return;
      setState(next);
      const first = next.people[0] ?? DEFAULT_PEOPLE[0];
      setSelectedPerson(first);
      setForm((prev) => ({ ...prev, person: first }));
      setLoaded(true);
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void persistSharedState(state);
    }, 400);
    return () => window.clearTimeout(timer);
  }, [loaded, state]);

  useEffect(() => {
    if (state.people.includes(selectedPerson)) return;
    const next = state.people[0] ?? DEFAULT_PEOPLE[0];
    setSelectedPerson(next);
    setForm((prev) => ({ ...prev, person: next }));
  }, [state.people, selectedPerson]);
  ...
  const visibleRecords = useMemo(() => {
    ...
    return [...state.records]
      ...
      .filter((record) => {
        if (!q) return true;
        const haystack = `${record.person} ${record.date} ${record.status} ${record.temperature} ${record.symptoms.join(" ")} ${record.otherSymptom} ${record.note}`.toLowerCase();
        return haystack.includes(q);
      })
      .sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
  }, [dateFilter, query, state.records]);

  const dayRecords = useMemo(() => state.records.filter((record) => record.date === dateFilter), [dateFilter, state.records]);
  const donePeople = new Set(dayRecords.map((record) => record.person));
  const missingPeople = state.people.filter((person) => !donePeople.has(person));
  const concernCount = state.records.filter((record) => isConcern(record)).length;
  const attendanceCount = dayRecords.filter((record) => record.status === "出勤").length;
  const leaveCount = dayRecords.filter((record) => record.status !== "出勤").length;

  const saveRecord = (next: Omit<RecordItem, "id" | "updatedAt">) => {
    setState((prev) => {
      ...
      const rest = prev.records.filter((record) => !(record.person === next.person && record.date === next.date));
      return { ...prev, records: [item, ...rest] };
    });
  };
  ...
  const removePerson = (name: string) => {
    ...
    if (selectedPerson === name) {
      const next = state.people.find((person) => person !== name) ?? DEFAULT_PEOPLE[0];
      setSelectedPerson(next);
      setForm((prev) => ({ ...prev, person: next }));
    }
  };
  ...
  const clearAll = () => {
    ...
    setForm(emptyForm(DEFAULT_PEOPLE[0]));
    setNewPerson("");
    setQuery("");
    setDateFilter(today());
  };
        ...
        <section className="rounded-[2rem] bg-gradient-to-br from-sky-500 via-sky-400 to-cyan-300 p-6 text-white shadow-lg sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <Card title="登録人数" value={state.people.length} subtitle="名前の追加・削除ができます" />
              <Card title="今日の出勤" value={attendanceCount} subtitle={`対象日: ${dateFilter}`} />
              <Card title="要確認" value={concernCount} subtitle="37.5℃以上 or 症状あり" />
            </div>
          </div>
        </section>
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                ...
                />
                <button type="button" onClick={addPerson} className="rounded-2xl bg-sky-600 px-5 py-3 font-semibold text-white transition hover:bg-sky-700">
                  追加
                </button>
              </div>
              ...
              >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {STATUS_OPTIONS.map((status) => (
                      <ChipButton key={status} active={form.status === status} onClick={() => setForm((prev) => ({ ...prev, status }))}>
                        {status}
                      </ChipButton>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-4 pt-2 sm:flex-row sm:justify-between">
                  ...
                  <div className="flex items-end gap-3 sm:self-end">
                    <button
                      type="button"
                      onClick={() => setForm(emptyForm(selectedPerson || state.people[0] || DEFAULT_PEOPLE[0], form.date))}
                      className="rounded-2xl border border-slate-300 px-5 py-3 font-semibold text-slate-700 transition hover:bg-slate-50"
                    >
                      クリア
                    </button>
                    <button type="submit" className="rounded-2xl bg-slate-900 px-5 py-3 font-semibold text-white transition hover:bg-slate-800">
                      登録する
                    </button>
                  </div>
                </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                <Card title="登録人数" value={state.people.length} />
                <Card title="対象日の記録済み" value={`${donePeople.size}/${state.people.length}`} subtitle="休暇系は未入力に含めません" />
                <Card title="未入力人数" value={missingPeople.length} />
                <Card title="出勤 / 休暇系" value={`${attendanceCount} / ${leaveCount}`} />
              </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              <div className="mt-4 space-y-3">
                {visibleRecords.length ? (
                    ...
                    <div key={record.id} className="rounded-2xl border border-slate-200 p-4">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {record.symptoms.length ? (
                          record.symptoms.map((item) => (
                            <span key={item} className="rounded-full bg-amber-50 px-2.5 py-1 text-xs text-amber-700 ring-1 ring-amber-200">
                              {item}
                            </span>
                          ))
                        ) : (
                  ...
                  <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">条件に一致する記録がありません。</div>
                )}
              </div>
            </div>

            <button type="button" onClick={clearAll} className="w-full rounded-2xl border border-rose-200 bg-rose-50 px-5 py-3 font-semibold text-rose-700 transition hover:bg-rose-100">
              全削除
            </button>
          </aside>
        </section>
